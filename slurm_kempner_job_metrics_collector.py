...
class SlurmJobNodeCollector(Collector):
    ...
    def collect(self):
        ...
        job_details = GaugeMetricFamily('slurm_job_details', 'Comprehensive SLURM job information from sacct', 
                                      labels=['job_id', 'job_id_raw', 'user', 'partition', 'account', 'state', 'tres_cpu', 'tres_mem', 'tres_gres', 'start_time', 'end_time', 'elapsed', 'alloc_tres', 'node_list', 'ncpus', 'req_cpus'])
        ...
        try:
            job_output = self.run_cmd(['timeout', '-s', '9', '60s', '/usr/bin/sacct', 
                                     ...
                                     '--format=JobID,JobIDRaw,User,Partition,Account,State,AllocCPUS,ReqMem,ReqTRES,Start,End,Elapsed,AllocTRES,NodeList,NCPUs,ReqCPUS',
                                     '--starttime=today'])
            partition_counts = {}
            ...
            for line in job_output.splitlines():
                if line.strip():
                    ...
                    if len(parts) >= 16:
                        job_id, job_id_raw, user, partition, account, state, cpu, memory, tres, start_time, end_time, elapsed, alloc_tres, node_list, ncpus, req_cpus = parts[0:16]
                        ...
                        job_details.add_metric([
                            job_id, job_id_raw, user, partition, account, state, 
